...
languages = {
    "zh": {
        ...
        "columns_stats": ["Jarque - Bera 统计量", "P 值", "结果解读"],
        ...
        "normal_dist_xlabel": "数值",
        "normal_dist_ylabel": "频率",
        ...
        "table_header_colname": "列名",
        "analysis_title": "Jarque-Bera检验结果分析"
    },
    "en": {
        ...
        "normal_dist_title": "{col_name}: mu = {mu:.2f}, std = {std:.2f}",
        "normal_dist_xlabel": "Value",
        "normal_dist_ylabel": "Frequency",
        ...
        "table_header_colname": "Column Name",
        "analysis_title": "Jarque-Bera Test Result Analysis"
    }
}
        ...
        try:
            ...
            for col_idx in range(1, columns + 1):
                ...
                if column_values:
                    ...
                    values = [jb_statistic, p_value, interpretation]
                    ...
                    plt.title(title)
                    plt.xlabel(languages[self.current_language]["normal_dist_xlabel"])
                    plt.ylabel(languages[self.current_language]["normal_dist_ylabel"])
                    ...
                    # 绘制 QQ 图
                    plt.figure()
                    stats.probplot(column_values, dist=stats.norm, sparams=(mu, std), plot=plt)
            ...
            headers = [
                languages[self.current_language]["table_header_colname"],
                languages[self.current_language]["columns_stats"][0],  # Jarque-Bera统计量
                languages[self.current_language]["columns_stats"][1],  # P值
                languages[self.current_language]["columns_stats"][2]  # 结果解读
            ]
                ...
                for row in data:
                    ...
                    for i, value in enumerate(row):
                        ...
                        if isinstance(value, float):
                            row_cells[i].text = f"{value:.4f}"  # 保留4位小数
                        else:
                            row_cells[i].text = str(value)

                # 调整表格列宽（可选）
                for col in table.columns:
                    col.width = Inches(2)  # 设置列宽为2英寸
