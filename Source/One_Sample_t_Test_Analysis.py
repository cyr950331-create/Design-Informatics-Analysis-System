...
languages = {
    'zh': {
        ...
        'population_mean_label': "总体均值:",
        'invalid_mean': "总体均值必须是有效的数值",
        'explanation': {
        ...
        'interpretation': {
            ...
            "样本均值": "反映了样本数据的平均水平。",
            "样本标准差": "反映了样本数据的离散程度。",
            "效应量(Cohen's d)": "|d| < 0.2为微小效应，0.2 ≤ |d| < 0.5为小效应，0.5 ≤ |d| < 0.8为中等效应，|d| ≥ 0.8为大效应。"
        }
    },
    'en': {
        ...
        'invalid_mean': "Population mean must be a valid number",
        'explanation': {
        ...
        'interpretation': {
            ...
            "Effect Size (Cohen's d)": "|d| < 0.2 is trivial, 0.2 ≤ |d| < 0.5 is small, 0.5 ≤ |d| < 0.8 is medium, |d| ≥ 0.8 is large."
        }
    }
}
...
class OneSampleTTestAnalysisApp:
    ...
    def analyze_file(self):
        ...
        # 读取用户输入的总体均值（新增部分）
        try:
            self.population_mean = float(self.population_mean_entry.get())
        except ValueError:
            self.result_label.config(text=languages[self.current_language]["invalid_mean"])
            return
        ...
        try:
            ...
            for column in numerical_df.columns:
                ...
                sample_std = data.std()
                effect_size_d = (sample_mean - self.population_mean) / sample_std

                # 计算总体均值的95%置信区间
                confidence_interval_popmean = stats.t.interval(0.95, degrees_of_freedom, loc=sample_mean,
                                                               scale=stats.sem(data))
                # 计算样本均值与总体均值差异的置信区间
                mean_diff = sample_mean - self.population_mean
                confidence_interval_diff = (mean_diff - (confidence_interval_popmean[1] - sample_mean),
                                            mean_diff + (sample_mean - confidence_interval_popmean[0]))
                confidence_interval = confidence_interval_diff

                results.append([
                    column, t_stat, degrees_of_freedom, p_value, confidence_interval,
                    sample_mean, sample_std, effect_size_d
                ])
            ...
            if save_path:
                ...
                # 添加标题
                doc.add_heading(
                    '单样本 t 检验分析结果' if self.current_language == 'zh' else 'One-Sample t-Test Analysis Results',
                    0)

                # 添加分析结果表格（只包含数据部分）
                doc.add_heading(
                    '分析结果' if self.current_language == 'zh' else 'Analysis Results',
                    level=1)
                ...
                # 添加解释说明（使用项目符号列表）
                doc.add_heading(
                    '解释说明' if self.current_language == 'zh' else 'Explanations',
                    level=1)
                ...
                # 添加结果解读（使用项目符号列表）
                doc.add_heading(
                    '结果解读' if self.current_language == 'zh' else 'Interpretations',
                    level=1)
                ...
                for column in numerical_df.columns:
                    ...
                    plt.ylabel('值' if self.current_language == 'zh' else 'Value')
                    plt.axhline(y=self.population_mean, color='r', linestyle='--',
                                label='总体均值' if self.current_language == 'zh' else 'Population Mean')
                    plt.legend()
    ...
    def switch_language(self, event):
        ...
        self.population_mean_label.config(text=languages[self.current_language]["population_mean_label"])
    ...
    def create_ui(self):
        ...
        # 新增：总体均值输入框
        mean_frame = ttk.Frame(frame)
        mean_frame.pack(pady=5)
        self.population_mean_label = ttk.Label(mean_frame,
                                               text=languages[self.current_language]["population_mean_label"])
        self.population_mean_label.pack(side=tk.LEFT)
        self.population_mean_entry = ttk.Entry(mean_frame, width=10)
        self.population_mean_entry.insert(0, "0")  # 默认值为0
        self.population_mean_entry.pack(side=tk.LEFT, padx=5)
