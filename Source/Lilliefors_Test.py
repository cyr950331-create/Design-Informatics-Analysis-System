...
languages = {
    "zh": {
        ...
        "lilliefors_statistic": "Lilliefors 统计量",  # 新增
        "p_value": "P值",  # 新增
        "columns_stats": ["lilliefors_statistic", "p_value", "结果解读"],  # 修改
        ...
        "normal_dist_title": "{}: 均值 = {:.2f}, 标准差 = {:.2f}",
        "value_label": "数值",
        "frequency_label": "频率",
        "pp_plot_title": "{} PP图",
        "theoretical_cdf_label": "理论累积分布函数",
        "empirical_cdf_label": "经验累积分布函数",
        "qq_plot_title": "{} QQ图",
        "column_name_header": "列名",
        "data_points_label": "数据点",
        "reference_line_label": "参考线",
        "theoretical_quantiles_label": "理论分位数",
        "sample_quantiles_label": "样本分位数",
    ...
    "en": {
        ...
        "lilliefors_statistic": "Lilliefors Statistic",
        "p_value": "P-value",
        "columns_stats": ["lilliefors_statistic", "p_value", "Result Interpretation"],  # 修改
        ...
        "normal_dist_title": "{}: mean = {:.2f}, std = {:.2f}",
        "value_label": "Value",
        "frequency_label": "Frequency",
        "pp_plot_title": "{} PP Plot",
        "theoretical_cdf_label": "Theoretical CDF",
        "empirical_cdf_label": "Empirical CDF",
        "qq_plot_title": "{} QQ Plot",
        "column_name_header": "Column Name",
        "data_points_label": "Data Points",
        "reference_line_label": "Reference Line",
        "theoretical_quantiles_label": "Theoretical Quantiles",
        "sample_quantiles_label": "Sample Quantiles",
        "result_interpretation": "Results Interpretation"
    }
}
        ...
        try:
            ...
            for col_idx in range(1, columns + 1):
                ...
                if column_values:
                    ...
                    lilliefors_statistic, p_value = lilliefors(column_values, dist='norm')
                    ...
                    values = [lilliefors_statistic, p_value, interpretation]
                    ...
                    title = languages[self.current_language]["normal_dist_title"].format(col_name, mu, std)
                    plt.title(title)
                    ...
                    plt.xlabel(languages[self.current_language]["theoretical_cdf_label"])
                    plt.ylabel(languages[self.current_language]["empirical_cdf_label"])
                    ...
                    plt.figure()
                    # 生成QQ图数据并手动绘制，增加更多控制
                    res = stats.probplot(column_values, dist="norm")
                    # 绘制数据点
                    plt.scatter(res[0][0], res[0][1], alpha=0.7)
                    # 绘制参考线
                    plt.plot(res[1][0], res[1][1], 'r--')
                    # 设置标题和标签（使用多语言字典）
                    plt.title(languages[self.current_language]["qq_plot_title"].format(col_name))
                    plt.xlabel(languages[self.current_language]["theoretical_quantiles_label"])
                    plt.ylabel(languages[self.current_language]["sample_quantiles_label"])
                    # 调整布局避免裁剪
                    plt.tight_layout()
                    # 保存图片
                    qqplot_path = os.path.splitext(file_path)[0] + f'_{col_name}_qqplot.png'
                    plt.savefig(qqplot_path, dpi=150)  # 增加分辨率
                    plt.close()

            columns_stats = [
                languages[self.current_language]["lilliefors_statistic"],
                languages[self.current_language]["p_value"],
                languages[self.current_language]["result_interpretation"]  # 确保已有这个键
            ]
