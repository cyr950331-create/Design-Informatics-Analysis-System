...
languages = {
    ...
    'en': {
        'title': "Binary Logit Regression Robustness test",
...
class BinaryLogitRegressionAnalysisApp:
    ...
    def open_template_excel(self):
        try:
            ...
            excel_path = os.path.join(parent_dir, "Sample_data", "TestData.xlsx")
    ...
    def analyze_file(self):
        ...
        try:
            ...
            # ---------------------- 新增：随机种子稳健性检验 ----------------------
            # 要测试的随机种子（稳健性检验）
            random_seeds = [10, 20, 30, 42, 50, 60, 70]
            # 用于保存结果
            robustness_results = []
            ...
            for seed in random_seeds:
                ...
                # 训练模型（statsmodels）
                X_train_robust_with_const = sm.add_constant(X_train_robust)
                logit_model_robust = sm.Logit(y_train_robust, X_train_robust_with_const).fit(
                    disp=0, method='bfgs', maxiter=1000
                )

                # 预测
                X_test_robust_with_const = sm.add_constant(X_test_robust)
                y_pred_proba_robust = logit_model_robust.predict(X_test_robust_with_const)
                y_pred_robust = (y_pred_proba_robust >= 0.5).astype(int)

                # 计算指标（添加zero_division=0避免除零错误）
                auc_robust = roc_auc_score(y_test_robust, y_pred_proba_robust)
                acc_robust = accuracy_score(y_test_robust, y_pred_robust)
                prec_robust = precision_score(y_test_robust, y_pred_robust, zero_division=0)
                rec_robust = recall_score(y_test_robust, y_pred_robust, zero_division=0)

                # 保存
                robustness_results.append({
                    'random_state': seed,
                    'AUC': round(auc_robust, 4),
                    'Accuracy': round(acc_robust, 4),
                    'Precision': round(prec_robust, 4),
                    'Recall': round(rec_robust, 4)
                })

            # 整理稳健性结果为DataFrame
            df_robust = pd.DataFrame(robustness_results)
            # 计算均值与范围（稳健性证明用）
            auc_mean = round(df_robust['AUC'].mean(), 4)
            auc_range = (round(df_robust['AUC'].min(), 4), round(df_robust['AUC'].max(), 4))
            acc_mean = round(df_robust['Accuracy'].mean(), 4)
            acc_range = (round(df_robust['Accuracy'].min(), 4), round(df_robust['Accuracy'].max(), 4))
            ...
            if save_path:
                ...
                # ---------------------- 新增：Word中添加稳健性检验结果 ----------------------
                doc.add_heading('5. 稳健性检验（随机种子）', level=2)
                # 添加稳健性检验表格
                robust_table = doc.add_table(rows=df_robust.shape[0] + 1, cols=df_robust.shape[1])
                # 设置表头
                hdr_cells = robust_table.rows[0].cells
                for col_idx, col_name in enumerate(df_robust.columns):
                    hdr_cells[col_idx].text = col_name
                # 填充数据
                for row_idx in range(df_robust.shape[0]):
                    row_cells = robust_table.rows[row_idx + 1].cells
                    for col_idx, value in enumerate(df_robust.iloc[row_idx]):
                        row_cells[col_idx].text = str(value)

                # 添加均值与范围统计
                robust_stats = doc.add_paragraph()
                robust_stats.add_run('稳健性统计（多随机种子）：\n').bold = True
                robust_stats.add_run(f"AUC 均值: {auc_mean}, 范围: {auc_range[0]} ~ {auc_range[1]}\n")
                robust_stats.add_run(f"Accuracy 均值: {acc_mean}, 范围: {acc_range[0]} ~ {acc_range[1]}")
