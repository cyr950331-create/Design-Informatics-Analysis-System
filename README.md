...
- **`Source/Toolkit.py`** — the hub that registers every analysis module in `MODULE_MAP` and renders them as searchable buttons in the GUI.
...
1. Download `DIAS.exe` from the [OneDrive link](https://1drv.ms/u/c/56791b21f8f8c84a/IQAmT6K5BrNUQoB1l56YLDz-ASQ3cBWppZEmBcyexUCMz_U?e=18fNCL).
2. Double-click `DIAS.exe` to launch.
3. Follow the built-in user manual.
...
1. **Open a dataset** — load a `.csv` or `.xlsx` file from the main window.
2. **Clean data** — use the built-in data-cleaning dialog to handle missing values and normalize column names.
3. **Select a method** — open the Toolkit and pick from 77 analysis methods (searchable, bilingual).
4. **Run the analysis** — DIAS automatically configures parameters, checks assumptions, and runs the test.
5. **Export results** — a Word report and charts are saved to your working directory.
...
| Demo Video | [YouTube](https://youtu.be/cFtxfOEURPE) |
...
| Layer | Libraries |
|-------|-----------|
| GUI | [ttkbootstrap](https://ttkbootstrap.readthedocs.io/) 1.12 |
...
1. Create `Source/Your_Method_Analysis.py` following the pattern of existing modules (ttkbootstrap UI + analysis logic + report generation).
2. Register it in `MODULE_MAP` inside `Source/Toolkit.py`.
...
Email: [yingrui.chi@unicam.it](mailto:yingrui.chi@unicam.it)
